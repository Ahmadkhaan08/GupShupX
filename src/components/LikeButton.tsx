...
const vote=async (voteValue:number,postId:number,userId:string) => {
    const {data:existingVote}=await supabase.from("votes").select("*").eq("post_id",postId).eq("user_id",userId).maybeSingle()
...
export const LikeButton=({postId}:Props)=>{
    ...
    mutationFn:(voteValue:number)=>{
     ...
     // Optimistically update the votes
        ...
        // Update the cache with the optimistic votes
        queryClient.setQueryData(['votes', postId], updatedVotes);
